...
def batched_non_quaternion_state_preturb(states, rng, *, num_batch_dims: int=1):
    ...
    states = states.at[:, :3].set(states[:, :3] + jax.random.uniform(subkey1, (N, 3), minval=-1.0, maxval=1.0) * 0.004)
    ...
    if len(states[0]) > 7:
        states = states.at[:, 7:].set(states[:, 7:] + jax.random.uniform(subkey2, (N, len(states[0])-7), minval=-1.0, maxval=1.0) * 0.004)
